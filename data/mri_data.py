...
class CmrxReconInferenceSliceDataset(torch.utils.data.Dataset):
    ...
    def _load_volume(self, path):
        ...
        if self.year == 2023:
            ...
            # For 2025 dataset with FullSample data, masks are generated on-the-fly
            # No pre-computed mask files are needed
            mask = None  # Will be generated by the transform's mask_func
        else:
...
# --- ADD THESE LOGGING LINES ---
    ...
    def __getitem__(self, idx):
        ...
        if self.transform is None:
            ...
            # For 2025 data with on-the-fly mask generation, pass mask=None
            # The transform will generate the mask using its mask_func
            if self.year == 2025 and mask is None:
                sample = self.transform(kspace, None, None, self.attrs, _path, slice_idx, self.current_num_t, self.current_num_z)
            else:
