...
from data.subsample import MaskFunc, CmrxRecon24MaskFunc, CmrxRecon25MaskFunc, PoissonDiscMaskFunc
...
def apply_mask(
    ...
    elif isinstance(mask_func, CmrxRecon25MaskFunc):
        # CmrxRecon25MaskFunc requires acc_factor parameter
        # Use mask_acc attribute if available, otherwise use first acceleration
        acc_factor = getattr(mask_func, 'mask_acc', None)
        if acc_factor is None:
            acc_factor = getattr(mask_func, 'accelerations', [8])[0]
        mask, num_low_frequencies = mask_func(shape, offset, seed, slice_idx, num_t, num_slc, acc_factor)
    ...
    def __call__(
        ...
        if self.mask_func is not None:
            ...
            if mask is not None:
                ...
                # For 2025 data with on-the-fly mask generation, mask was already generated by apply_mask
                # This should not happen, but handle gracefully
                mask_torch = torch.zeros_like(kspace_torch[..., 0:1])
